...
import { useUpdateEvent, useManagedTargets } from '../../hooks/useEvents';
...
const EditEventModal = ({ event, onClose }) => {
  const { mutate: updateEvent, isLoading } = useUpdateEvent(event.id);
  ...
  // 1. Initialize states based on the passed event
  const isEventAllDay = event.start_time === '00:00:00' && event.end_time === '23:59:59';
  const hasNoTargets = !event.targets || event.targets.length === 0;

  const [step, setStep] = useState(0); 
  const [isGlobal, setIsGlobal] = useState(hasNoTargets);
  ...
  const onPickerChange = (pickerEvent, selectedDate) => {
    setShowPicker(false);
  ...
  const [targetGroups, setTargetGroups] = useState({
    program: event.targets?.some(t => t.targetable_type === 'program') || false,
    section: event.targets?.some(t => t.targetable_type === 'section') || false
  });
  ...
  const [form, setForm] = useState({
    ...
    start_time: isEventAllDay ? '08:00' : (event.start_time?.substring(0, 5) || '08:00'),
    end_time: isEventAllDay ? '17:00' : (event.end_time?.substring(0, 5) || '17:00'),
    ...
    updateEvent(payload, {
      ...
      onError: (err) => {
        ...
        Toast.show({
          ...
          text2: serverMsg || 'Check your network connection.',
          position: 'top',
        });
      }
    });
          ...
          <View className="flex-row justify-between items-center mb-6">
            <View>
              <Text style={{ fontFamily: 'Poppins_600SemiBold' }} className="text-xl text-slate-900">
                {step === 0 ? "Edit Details" : "Target Audience"}
              </Text>
