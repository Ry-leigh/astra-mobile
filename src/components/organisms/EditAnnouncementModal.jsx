...
import { useUpdateAnnouncement, useAnnouncementTargets } from '../../hooks/useAnnouncements'; 

const EditAnnouncementModal = ({ announcement, onClose }) => {
  const { mutate: updateAnnouncement, isLoading } = useUpdateAnnouncement();
  ...
  // Pre-fill data and map targets
  useEffect(() => {
    if (announcement) {
      // 1. Map backend targets to frontend structure
      const mappedTargets = (announcement.targets || []).map(t => ({
        type: t.targetable_type,
        id: t.targetable_id
      }));

      // 2. Activate necessary target group checkboxes
      const groups = { role: false, program: false, section: false };
      ...
      setIsGlobal(mappedTargets.length === 0);
      setTargetGroups(groups);

      // 3. Set the form state
      setForm({
        title: announcement.title,
        description: announcement.description,
        require_acknowledgement: announcement.require_acknowledgement === 1 || announcement.require_acknowledgement === true,
  ...
  const handleFinalSubmit = () => {
    ...
    updateAnnouncement({ id: announcement.id, ...payload }, {
      onSuccess: () => {
        Toast.show({
          type: 'success',
          text1: 'Changes Saved!',
          text2: 'The announcement was updated.',
          position: 'top',
        });
        onClose();
      },
    ...
    <View className="flex-1 justify-end" style={{ paddingBottom: insets.bottom }}>
          ...
          <View className="flex-row justify-between items-center mb-6">
            <View>
              <Text style={{ fontFamily: 'Poppins_600SemiBold' }} className="text-xl text-slate-900">
                {step === 0 ? "Edit Info" : "Edit Target Audience"}
              </Text>
          ...
          <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
            {step === 0 ? (
              <View className="gap-y-4 pb-8">
                ...
                >
                  <View>
                    <Text style={{ fontFamily: 'Poppins_500Medium' }} className='text-slate-700'>Require Acknowledgement</Text>
                  </View>
              ...
              <View className="pb-8">
                 {errors.targets && (
                  <Text className="text-rose-500 text-xs mb-2 font-poppins">Target audience required</Text>
                )}
                <TouchableOpacity 
                ...
                >
                  <View className="flex-1 pr-4">
                    <Text style={{ fontFamily: 'Poppins_600SemiBold' }} className={isGlobal ? 'text-indigo-700' : 'text-slate-800'}>Global Announcement</Text>
                    <Text style={{ fontFamily: 'Poppins_400Regular' }} className="text-slate-500 text-xs">Visible to everyone in the institution.</Text>
                  </View>
                ...
                {!isGlobal && (
                  <View>
                    ...
                    <View className={`mb-4 bg-slate-50 p-4 rounded-3xl border ${errors.targets ? 'border-rose-500' : 'border-slate-100'}`}>
                      <TouchableOpacity className="flex-row items-center" onPress={() => toggleTargetGroup('role')}>
                      ...
                      {targetGroups.role && (
                        <View className="flex-row flex-wrap gap-2 mt-4 ml-9">
                          {availableRoles.map(role => (
                            <TouchableOpacity 
                              key={`role-${role.id}`} onPress={() => toggleTarget('role', role.id)}
                            ...
                            >
                              <Text style={{ fontFamily: 'Poppins_500Medium' }} className={isTargetSelected('role', role.id) ? 'text-indigo-700' : 'text-slate-600'}>{role.label}</Text>
                            </TouchableOpacity>
                          ))}
                        </View>
                      )}
                    ...
                    <View className={`mb-4 bg-slate-50 p-4 rounded-3xl border ${errors.targets ? 'border-rose-500' : 'border-slate-100'}`}>
                      <TouchableOpacity className="flex-row items-center" onPress={() => toggleTargetGroup('program')}>
                        ...
                        <Text style={{ fontFamily: 'Poppins_500Medium' }} className="text-slate-800 text-base">By Program</Text>
                      </TouchableOpacity>
                      ...
                      {targetGroups.program && (
                        <View className="flex-row flex-wrap gap-2 mt-4 ml-9">
                          {availablePrograms.map(prog => (
                            <TouchableOpacity 
                              key={`prog-${prog.id}`} onPress={() => toggleTarget('program', prog.id)}
                            ...
                            >
                              <Text style={{ fontFamily: 'Poppins_500Medium' }} className={isTargetSelected('program', prog.id) ? 'text-indigo-700' : 'text-slate-600'}>{prog.label}</Text>
                            </TouchableOpacity>
                          ))}
                        </View>
                      )}
                    ...
                    <View className={`mb-4 bg-slate-50 p-4 rounded-3xl border ${errors.targets ? 'border-rose-500' : 'border-slate-100'}`}>
                      <TouchableOpacity className="flex-row items-center" onPress={() => toggleTargetGroup('section')}>
                        ...
                        <Text style={{ fontFamily: 'Poppins_500Medium' }} className="text-slate-800 text-base">By Section</Text>
                      </TouchableOpacity>
                      ...
                      {targetGroups.section && (
                        <View className="flex-row flex-wrap gap-2 mt-4 ml-9">
                          {availableSections.map(sec => (
                            <TouchableOpacity 
                              key={`sec-${sec.id}`} onPress={() => toggleTarget('section', sec.id)}
                            ...
                            >
                              <Text style={{ fontFamily: 'Poppins_500Medium' }} className={isTargetSelected('section', sec.id) ? 'text-indigo-700' : 'text-slate-600'}>{sec.label}</Text>
                            </TouchableOpacity>
                          ))}
                        </View>
                      )}
      ...
      <View className="flex-row gap-3 p-6 bg-white">
        ...
        <TouchableOpacity onPress={step === 0 ? nextStep : handleFinalSubmit} disabled={isLoading} className={`flex-[2] p-4 rounded-2xl flex-row items-center justify-center ${isLoading ? 'bg-indigo-400' : 'bg-indigo-600'}`}>
          <Text style={{ fontFamily: 'Poppins_600SemiBold' }} className="text-white text-lg mr-2">{isLoading ? 'Saving...' : step === 0 ? 'Next' : 'Save Changes'}</Text>
